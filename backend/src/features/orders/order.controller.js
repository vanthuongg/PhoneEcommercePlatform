  ...
  try {
    ...
    if (note && note.trim() !== '') {
      order.messages.push({
        ...
        senderName: req.user.name || (req.user.role === 'admin' ? 'Quản trị viên' : 'Nhân viên CSKH'),
        content: note.trim(),
        createdAt: new Date(),
      });
    }
...
const updatePaymentStatus = async (req, res) => {
  try {
    ...
    order.timeline.push({
      ...
      description: note || `Trạng thái thanh toán được cập nhật thành: ${paymentStatus === 'paid' ? 'Đã thanh toán' : paymentStatus === 'failed' ? 'Thất bại' : paymentStatus === 'refunded' ? 'Đã hoàn tiền' : 'Chờ thanh toán'}`,
      time: new Date(),
      completed: true,
    });
...
// @desc    Add message/note to order (Staff, Admin, Customer)
...
const addOrderMessage = async (req, res) => {
  try {
    ...
    const senderRole = req.user.role === 'customer' ? 'customer' : (req.user.role === 'admin' ? 'admin' : 'staff');
    const senderName = req.user.name || (senderRole === 'customer' ? 'Khách hàng' : 'Nhân viên TechPhone');
    ...
    if (senderRole !== 'customer' && order.user) {
      await Notification.create({
        user: order.user._id || order.user,
        title: `💬 Tin nhắn mới từ đơn hàng #${order.orderCode}`,
    ...
    res.json({ success: true, message: 'Gửi tin nhắn thành công', data: order });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};

module.exports = { createOrder, getOrders, getOrderById, updateOrderStatus, updatePaymentStatus, cancelOrder, addOrderMessage };
