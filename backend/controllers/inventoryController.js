...
exports.createStockTransaction = async (req, res) => {
  try {
    ...
    // Xác định trước tồn kho hiện tại (beforeStock) và thông tin biến thể
    if (variantId) {
    ...
    // Tính toán tồn kho sau giao dịch (afterStock) và lượng thay đổi (changeQty)
    ...
    // Cập nhật tồn kho vào Product / Variant trong cơ sở dữ liệu
    if (variantId) {
      const variant = product.variants && product.variants.id(variantId);
      const color = product.colors && product.colors.id(variantId);
      ...
      // Cập nhật lại tổng tồn kho chung của sản phẩm
    ...
    // Ghi nhận Thẻ kho / Lịch sử xuất nhập
