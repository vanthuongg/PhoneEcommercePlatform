...
const faqResponses = {
  ...
  payment: {
    ...
    response: '💳 **Dạ, bên em hỗ trợ các hình thức thanh toán sau:**\n• 💵 Tiền mặt khi nhận hàng (COD).\n• 🏦 Chuyển khoản ngân hàng.\n• 💳 Quẹt thẻ / Ví MoMo / VNPay an toàn tuyệt đối.\n\nAnh/chị chọn cách nào tiện nhất cho mình nhé!',
  },
  returns: {
...
const searchProducts = async (keywords, options = {}) => {
  ...
  if (keywords) {
    ...
    if (searchTerms.length > 0) {
      // Create a smart query: either match the whole phrase, or match ALL individual terms across name/brand/tags
      ...
      query.$or = [
        { name: { $regex: keywords, $options: 'i' } }, // Exact phrase in name
        { brand: { $regex: keywords, $options: 'i' } }, // Exact phrase in brand
        { $and: allTermsQuery } // Contains all words anywhere
      ];
    }
  }
  ...
  try {
    ...
    switch (intent) {
      ...
      default: {
        ...
        // Try category exact search first
        ...
        if (found.length > 0) {
          ...
          // Fallback: search individual keywords that are long enough
