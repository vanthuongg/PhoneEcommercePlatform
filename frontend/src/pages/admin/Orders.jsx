...
const nextStatus = { pending: 'confirmed', confirmed: 'processing', processing: 'shipping', shipping: 'delivered' };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Quản lý đơn hàng</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading ? (
                [...Array(5)].map((_, i) => <tr key={i}><td colSpan={8} className="px-4 py-4"><div className="h-4 bg-gray-200 rounded animate-pulse" /></td></tr>)
              ) : orders.map((order) => (
                <tr key={order._id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`badge ${order.paymentStatus === 'paid' ? 'badge-success' : order.paymentStatus === 'failed' ? 'badge-danger' : 'badge-warning'}`}>
                      {order.paymentStatus === 'paid' ? 'Đã TT' : order.paymentStatus === 'failed' ? 'Thất bại' : 'Chưa TT'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => setSelectedOrder(order)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Eye className="w-4 h-4" /></button>
                      {nextStatus[order.orderStatus] && (
                        <button
                          onClick={() => handleUpdateStatus(order._id, nextStatus[order.orderStatus])}
                          disabled={updating}
                          className="text-xs px-2 py-1 bg-primary-50 text-primary-700 hover:bg-primary-100 rounded-lg transition-colors font-medium"
                        >
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-slide-up">
            ...
            <div className="p-5 space-y-5">
              {/* Customer info */}
              <div>
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">Thông tin khách hàng</h3>
              ...
              {/* Items */}
              <div>
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">Sản phẩm</h3>
                  ...
                  {selectedOrder.items?.map((item) => (
                    <div key={item._id} className="flex gap-3 items-center p-3 bg-gray-50 dark:bg-gray-900 rounded-xl">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">x{item.quantity} • {formatPrice(item.price)}</p>
                      </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-4 text-sm space-y-2">
                <div className="flex justify-between text-gray-600 dark:text-gray-400"><span>Tạm tính</span><span>{formatPrice(selectedOrder.itemsTotal)}</span></div>
                <div className="flex justify-between text-gray-600 dark:text-gray-400"><span>Vận chuyển</span><span>{formatPrice(selectedOrder.shippingFee)}</span></div>
                <div className="flex justify-between font-bold text-primary-600 text-base pt-2 border-t"><span>Tổng</span><span>{formatPrice(selectedOrder.totalAmount)}</span></div>
              </div>

              {/* Payment Status update */}
              <div className="border-t pt-4 space-y-3">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Trạng thái thanh toán</h3>
              ...
              {/* Status update */}
              <div className="border-t pt-4 space-y-3">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Cập nhật trạng thái giao hàng</h3>
