            ...
            <button
              ...
              className={`p-2.5 rounded-full relative transition-all ${
                !isCustomerOrGuest 
                  ...
                  : 'text-slate-700 dark:text-slate-200 hover:text-primary-600 hover:bg-slate-100 dark:hover:bg-slate-800'
              }`}
              title="Thông báo"
            >
              <Bell size={20} />
              {unreadCount > 0 && (
                <span className={`absolute top-1 right-1 w-4 h-4 text-white font-semibold text-[9px] rounded-full flex items-center justify-center shadow-xs ${
                  !isCustomerOrGuest ? 'bg-indigo-600 animate-pulse' : 'bg-primary-600'
                }`}>
                  {unreadCount}
                </span>
              )}
            ...
            {showNotifDropdown && (
              <div className="absolute right-0 mt-3.5 w-80 sm:w-96 bg-white dark:bg-slate-900 rounded-2xl shadow-premium dark:shadow-premium-dark border border-slate-200/60 dark:border-slate-800/60 overflow-hidden z-50 animate-fade-in">
                <div className={`flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700/50 ${
                  !isCustomerOrGuest ? 'bg-slate-900 text-white dark:bg-slate-950' : 'bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white'
                }`}>
                  <span className="font-semibold text-sm">
                    {!isCustomerOrGuest ? `🛡️ Thông báo quản trị (${user?.role})` : 'Thông báo mới gần đây'}
                  </span>
                  {unreadCount > 0 && (
                    <button onClick={markAllAsRead} className={`text-xs hover:underline font-semibold ${!isCustomerOrGuest ? 'text-indigo-400' : 'text-primary-600'}`}>
                      Đọc tất cả
                    </button>
                  )}
                </div>
                <div className="max-h-80 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800">
                  {notifications.length > 0 ? (
                    notifications.slice(0, 5).map((n) => (
                      <div
                        key={n.id || n._id}
                        onClick={() => { markAsRead(n.id || n._id); setShowNotifDropdown(false); if (n.link) navigate(n.link); else navigate('/profile?tab=notifications'); }}
                        className={`p-4 hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer transition-colors ${
                          !n.isRead 
                            ? (!isCustomerOrGuest ? 'bg-indigo-50/70 dark:bg-indigo-950/30 border-l-4 border-indigo-600' : 'bg-primary-50/50 dark:bg-primary-900/10') 
                            : ''
                        }`}
                      >
                    ...
                    <div className="p-8 text-center text-sm text-slate-400">
                      {!isCustomerOrGuest ? 'Chưa có thông báo quản trị nào' : 'Không có thông báo nào'}
                    </div>
                  )}
                </div>
                <div className={`p-3 text-center border-t border-slate-100 dark:border-slate-700/50 ${
                  !isCustomerOrGuest ? 'bg-slate-900 dark:bg-slate-950' : 'bg-slate-50 dark:bg-slate-800'
                }`}>
                  <Link
                    to="/profile?tab=notifications"
                    onClick={() => setShowNotifDropdown(false)}
                    className={`text-xs font-semibold hover:underline ${!isCustomerOrGuest ? 'text-indigo-400' : 'text-primary-600'}`}
                  >
                    {!isCustomerOrGuest ? 'Xem trung tâm thông báo nội bộ →' : 'Xem tất cả thông báo'}
                  </Link>
                </div>
              </div>
            )}
